...
public class AdminFuncionariosController : ControllerBase
{
    ...
    public async Task<IActionResult> Listar(CancellationToken ct)
        => Ok(await _listar.Executar(ct));

    [HttpGet("{id:guid}")]
    public async Task<IActionResult> Obter(Guid id, CancellationToken ct)
        => Ok(await _obter.Executar(id, ct));
    ...
    {
        await _alterarStatus.Executar(id, ativo: true, ct);
        return NoContent();
    }
    ...
    {
        await _alterarStatus.Executar(id, ativo: false, ct);
        return NoContent();
    }
}
