...
public class SegurancaUseCasesTests
{
    ...
    {
        var cliente = new Cliente(new DocumentoCpfCnpj("12345678909"), "Cliente", new Contato("c@teste.com", "11999999999"));
        var repo = new Mock<ICadastroRepository>();
        repo.Setup(x => x.ObterClientePorDocumento("12345678909", It.IsAny<CancellationToken>()))
            .ReturnsAsync(cliente);

        var useCase = new AutenticarClienteUseCase(repo.Object);

        var resultado = await useCase.Executar("123.456.789-09", CancellationToken.None);
    ...
    public async Task AutenticarCliente_ComCpfInexistente_DeveFalhar()
    {
        ...
        var ex = await Assert.ThrowsAsync<OficinaException>(() => useCase.Executar("12345678909", CancellationToken.None));
    ...
    {
        var funcionario = new Funcionario("Maria", "12345678909", "hash", PerfilUsuarioInterno.Funcionario);
        var repo = new Mock<IFuncionarioRepository>();
        var password = new Mock<IPasswordHashService>();
        repo.Setup(x => x.ObterPorCpf("12345678909", It.IsAny<CancellationToken>())).ReturnsAsync(funcionario);
        password.Setup(x => x.Verificar("hash", "SenhaTeste!123")).Returns(true);

        var useCase = new AutenticarFuncionarioUseCase(repo.Object, password.Object);

        var resultado = await useCase.Executar("12345678909", "SenhaTeste!123", CancellationToken.None);
    ...
    public async Task AutenticarFuncionario_ComSenhaInvalida_DeveFalhar()
    {
        ...
        var ex = await Assert.ThrowsAsync<OficinaException>(() => useCase.Executar("12345678909", "errada", CancellationToken.None));
        Assert.Equal(401, ex.StatusHttp);
    }
    ...
    public async Task AutenticarFuncionario_Inativo_DeveFalhar()
    {
        ...
        var ex = await Assert.ThrowsAsync<OficinaException>(() => useCase.Executar("12345678909", "SenhaTeste!123", CancellationToken.None));
        Assert.Equal(401, ex.StatusHttp);
    }
    ...
    public async Task CriarFuncionario_DevePermitirPerfisInternosESemExporSenhaHash(string perfil)
    {
        var repo = new Mock<IFuncionarioRepository>();
        var password = new Mock<IPasswordHashService>();
        password.Setup(x => x.Hash("SenhaTeste!123")).Returns("hash");
        repo.Setup(x => x.ExistePorCpf("12345678909", It.IsAny<CancellationToken>())).ReturnsAsync(false);

        var useCase = new CriarFuncionarioUseCase(repo.Object, password.Object);

        var response = await useCase.Executar(
            new CriarFuncionarioRequest("Maria Oficina", "12345678909", "SenhaTeste!123", perfil),
